...
* Построение покрытия: `go test -coverpgo test -coverprofile=cover.out && go tool cover -html=cover.out -o cover.htmlrofile=cover.out && go tool cover -html=cover.out -o cover.html`
